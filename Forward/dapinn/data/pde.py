...
class PDE(Data):
    ...
    def train_points(self):
        ...
        if self.mode=="copy2" :
            x_2 = X[:,0:1]
            X =np.insert(X,[1],x_2,axis=1)

        if self.mode == "copy2xy":
            x_2 =X[:,0:1]
            y_2=X[:,1:2]
            X_N = np.hstack((x_2, y_2))
        ...
        if self.mode=="x3t" :
            ...
            X_N = np.hstack((x_3, x_2))
            X =np.insert(X,[2],X_N,axis=1)
            X[:,[1,3]]=X[:,[3,1]]

        if self.mode=="fourier t" :
            sin = np.sin(X[:, 0:1])
            cos = np.cos(X[:, 0:1])
            X_N = np.hstack((cos, sin))
            X = np.insert(X, [2], X_N, axis=1)
            X[:, [1, 3]] = X[:, [3, 1]]
        if self.mode == "x3tt":
            t_2= X[:, 1:2] * X[:, 1:2]
...
class TimePDE(PDE):
    ...
    def __init__(
        ...
        solution=None,
        num_test=None,
        auxiliary_var_function=None,mode=None,
    ):
        ...
        super().__init__(
            ...
            solution=solution,
            num_test=num_test,
            auxiliary_var_function=auxiliary_var_function,mode=mode
        )
    ...
    def train_points(self):
        ...
        if self.num_initial > 0:
            ...
            if self.mode == "x2t":
                x_2 = tmp[:, 0:1] * tmp[:, 0:1]
                tmp = np.insert(tmp, [2], x_2, axis=1)
                tmp[:, [1, 2]] = tmp[:, [2, 1]]

            if self.mode == "x3t":
                x_2 = tmp[:, 0:1] * tmp[:, 0:1]
                x_3 = tmp[:, 0:1] * tmp[:, 0:1] * tmp[:, 0:1]
                x_N = np.hstack((x_3, x_2))
                tmp = np.insert(tmp, [2], x_N, axis=1)
                tmp[:, [1, 3]] = tmp[:, [3, 1]]
            ...
            if self.mode == "fourier tt":
                t_2 = tmp[:, 1:2] * tmp[:, 1:2]
                sin = np.sin(np.pi*tmp[:, 0:1])
                cos = np.cos(np.pi*tmp[:, 0:1])
                x_N = np.hstack((cos, t_2,sin))
                tmp = np.insert(tmp, [2], x_N, axis=1)
                tmp[:, [1, 4]] = tmp[:, [4, 1]]
            if self.mode == "Hermite":
                x_2 = tmp[:, 0:1] ** 2 - 1
                x_3 = tmp[:, 0:1] ** 3 - 3 * tmp[:, 0:1]
                x_N = np.hstack((x_3, x_2))
                tmp = np.insert(tmp, [2], x_N, axis=1)
                tmp[:, [1, 3]] = tmp[:, [3, 1]]
